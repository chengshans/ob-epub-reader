...
- **摘录导出** — 标注自动写入 Vault 中的 Markdown 摘录文件，支持四种可配置的摘录链接格式；摘录文件夹与文件名支持 `{filefolder}`、`{title}`、`{filename}` 占位符
...
在 **设置 → 摘录链接格式** 可选择四种预设。修改设置仅影响新标注；已有摘录需点击 **转换已有摘录链接 → 立即转换** 批量转换。
...
| 摘录链接格式 | 四种预设格式；见上文 | Callout + 标题链接 |
